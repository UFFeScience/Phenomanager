...
import com.uff.model.invoker.domain.Team;
...
public class PermissionRepositoryImpl implements PermissionRepositoryCustom {
	...
	public Permission findOneByUserAndEntityNameAndComputationalModelSlug(User user, String computationalModelSlug) {
		try {
			...
			restrictions.add(criteriaBuilder.and(criteriaBuilder.equal(from.join("computationalModel").get("slug"), computationalModelSlug)));
		    ...
		    Join<?, Permission> entityPermissions = subqueryPermissionRoot.join("computationalModel").join("permissions");
		    ...
		    Join<?, Permission> entityTeamUsersPermissions = subqueryTeamUsersRoot.join("computationalModel").join("permissions");

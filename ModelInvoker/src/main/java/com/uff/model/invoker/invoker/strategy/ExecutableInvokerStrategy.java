...
public class ExecutableInvokerStrategy extends ModelInvoker {
	...
	@Override
	public void runInCloud(ModelExecutor modelExecutor, 
			ModelResultMetadata modelResultMetadata, Connection connection) 
		...
		if (modelExecutor.getExecutorFileId() == null || "".equals(modelExecutor.getExecutorFileId())) {
			...
			modelResultMetadata.appendExecutionLog(String.format("Finished downloading modelExecutor [%s]", 
					modelExecutor.getTag()));
			modelResultMetadata.appendExecutionLog(String.format("Uploading modelExecutor [%s] to environment...", 
					modelExecutor.getTag()));
	...
	public void runInCluster(ModelExecutor modelExecutor, ModelResultMetadata modelResultMetadata, Connection connection) 
		...
		if (modelExecutor.getExecutorFileId() == null || "".equals(modelExecutor.getExecutorFileId()) || 
			...
			modelResultMetadata.appendExecutionLog(String.format("Finished uploading modelExecutor [%s] to environment",
					modelExecutor.getTag()));
			modelResultMetadata.appendExecutionLog(String.format("Uploading scratch file [%s] to environment...", 
					modelExecutor.getTag()));
			...
			modelResultMetadata = updateExecutionOutput(modelResultMetadata, "Finished uploading scratch file to environment");
			
			LogSaverWrapper logSaver = LogSaverWrapper.builder()
				.modelResultMetadata(modelResultMetadata)
				.modelResultMetadataService(modelResultMetadataService).build();
			
			modelResultMetadata = updateExecutionOutput(modelResultMetadata, String.format("Submiting Job [%s]:", modelExecutor.getJobName()));
